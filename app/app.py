...
def get_students():
    ...
    try:
        ...
        session = DBsession()
        response['pagination']['total'] = session.query(Student).count()
        ret = session.query(Student).order_by(Student.id.desc()).limit(
...
def get_student_by_id(id):
    try:
        ...
        session = DBsession()
        response['data'] = session.query(Student).filter_by(id=id).first().info()
        session.close()
        return jsonify(response)
    except:
        return
...
def delete_students():
    ...
    try:
        session = DBsession()
        session.query(Student).filter(Student.id.in_(request.json['ids'])).delete(synchronize_session = False)
        session.commit()
        session.close()
        return jsonify({
            'code': 200,
